...
for i in `find ./`; do
    # echo $i
    DATELINENUMBER=$(grep -n "Translation Date[ ]*|"  $i | cut -d: -f 1)
    if [ !  -z  $DATELINENUMBER  ]; then
        ...
        DATEINDEX=${PARTST[Translation_Date]}
        # printf '%s\n' "${PARTST[@]}"
...
for i in "${SORTEDDATES[@]}"; do
    ...
    for j in "${SONGSSORTED[@]}"; do
        ...
        if [ !  -z  $SONGNUMBERSHORT  ]; then        
            sed -i "$line a $SONGNUMBERSHORT  | $DATER | [$SONGTITLE](gitsongs/$SONGNUMBER.md) | $HYMNCOUNT" indices/indexoftranslationdates.md
            let "line++"
        fi
    done
